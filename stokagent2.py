...
# --- שעונים עולמיים ויזואליים בלבד (ללא קריאות API לחסכון ב-Rate Limit) ---
...
def render_universal_stock_analysis(ticker_str, unique_key_prefix=""):
    ...
    # השארת אותיות באנגלית בלבד - מנקה כל גרש, סוגר או תווים נלווים שה-AI פלט
    ...
    if st.button(f"📊 לחץ להצגת גרף 6M ומדדים עבור {clean_ticker}", key=f"btn_chart_{clean_ticker}_{unique_key_prefix}"):
        with st.spinner(f"פונה לשרתי הבורסה למשוך נתונים עבור {clean_ticker}..."):
            try:
                ...
                if hist.empty:
                    st.error(f"❌ לא ניתן היה למשוך נתוני מחיר עבור '{clean_ticker}'. ודא שהסימול נכון (למשל: NVDA, XOM, TSLA) ושרשת ה-API של Yahoo פנויה.")
                    return
...
if st.button("🚀 הפעל רדאר לאיתור מניות פוטנציאליות", type="secondary"):
    if not api_key: st.warning("אנא הזן מפתח API בתפריט הצד.")
        ...
        with st.spinner("הסוכן מבצע מחקר מאקרו עולמי ומסנן ממצאים..."):
            ...
            - Ticker must be a clean string symbol (e.g., MP, INTC, NVDA, XOM). Do not append any other text to the ticker column.
                ...
                try:
                    ...
                    for line in lines:
                        if "|" in line and "TICKER" not in line:
                            ...
                            if len(parts) >= 4:
                                # חילוץ נקי לחלוטין של הטיקר מאינדקס 0 ומחיקת רעשי רקע
                                t_raw = str(parts).strip()
                                t_cleaned = re.sub(r'[^a-zA-Z]', '', t_raw).upper()
                                if t_cleaned and len(t_cleaned) < 7:
                                    parsed_rows.append({
                    ...
                    if parsed_rows:
                        ...
                        st.error("הסוכן הפיק את המידע אך מבנה השורות דורש הרצה חוזרת. נסה שנית.")
...
if st.session_state.radar_stocks_df is not None and not st.session_state.radar_stocks_df.empty:
    ...
    # 🎯 הפתרון המבודד והבטוח: תיבת טקסט חופשית ונקייה ללא תלות בפירוק השבור של ה-AI!
    st.write("")
    ...
    # הפעלת פונקציית הגרף המשנית והחסינה
...
if not st.session_state.watchlist:
    ...
    # הגנה מפני חסימת קצב בקירות המעקב הכלליות
    try:
        watchlist_df = scan_sector_fundamentals(st.session_state.watchlist)
        st.dataframe(watchlist_df, use_container_width=True, hide_index=True)
    except:
        st.caption("⚠️ שרתי יאהו חסמו זמנית את טבלת הסיכום הכללית. ניתן עדיין לבדוק מניות בודדות למטה.")
