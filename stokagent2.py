...
if "radar_tickers" not in st.session_state:
    st.session_state.radar_tickers = ["NVDA", "XOM", "INTC", "TSLA"] # מניות ברירת מחדל לרדאר
...
# =====================================================================
# רכיב א': רדאר אירועים גלובליים (טבלה נקייה לחלוטין)
# =====================================================================
...
if st.button("🚀 הפעל רדאר לאיתור מניות פוטנציאליות", type="secondary"):
    if not api_key: st.warning("אנא הזן מפתח API בתפריט הצד.")
        ...
        with st.spinner("הסוכן מבצע מחקר מאקרו עולמי ומפיק ממצאים..."):
            ...
            - Ticker must be a clean, unquoted string symbol (e.g., MP, INTC, NVDA, XOM). Do not add any special characters.
            ...
            if full_text:
                    ...
                    if "[DATA_START]" in full_text and "[DATA_END]" in full_text:
                        ...
                        for line in lines:
                            if "|" in line:
                                ...
                                if len(parts) >= 4:
                                    # חילוץ נקי מבוסס אינדקס 0 בלבד ומחיקת כל תו נלווה
                ...
                except: st.error("שגיאה קלה בעיבוד בלוק המניות. נסה שנית.")
if st.session_state.radar_stocks_df is not None and not st.session_state.radar_stocks_df.empty:
    ...
    # 🎯 שינוי קריטי: מעבר לתיבת בחירה נקייה (Selectbox) מתוך הרשימה המנוקה ללא שדות טקסט חופשיים!
...
if st.session_state.all_active_tickers:
    ...
    # 🎯 שינוי קריטי 2: מעבר ל-Selectbox מוגן ומובנה גם עבור הסורק הענפי
